...
function palindrome(string) {
  //replace all non-alphaneumeric characters and make all letters lowercase
  let strCode = string.replace(/[^a-z0-9]/gi, '').toLowerCase()
  // split string into an array - reverse the array and join it together to form a new string 
  let palidStr = strCode.split('').reverse().join('');
  // conditional to check if strCode and palidStr are palidromes 
  if(strCode === palidStr){
    return true
  }else{
    return false
